...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_prepare_map_bns.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hmeriann <hmeriann@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/30 21:36:49 by zu                #+#    #+#             */
/*   Updated: 2021/12/12 14:55:01 by hmeriann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_get_map_width(char **argv, t_map *map)
{
	...
	if (fd < 0)
	{
		ft_putstr_fd("open() is failed in width\n", 1);
		exit (EXIT_FAILURE);
	}
...
void	ft_get_map_height(char **argv, t_map *map)
{
	...
	if (fd < 0)
	{
		ft_putstr_fd("open() is failed in heihgt\n", 1);
		exit (EXIT_FAILURE);
	}
	map->height = 1;
	while (get_next_line(fd, &line))
...
void	ft_get_map(char **argv, t_map *map)
{
	...
	if (fd < 0)
	{
		ft_putstr_fd("open() is failed in parce\n", 1);
		exit (EXIT_FAILURE);
	}
	i = 0;
	while (get_next_line(fd, &line))
